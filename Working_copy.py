...
frm_checkboxes = Frame(root, width=800, height=300, bd=5, bg="aqua", relief="sunken")
frm_radiobuttons = Frame(root, width=800, height=300, bd=5, bg="grey", relief="sunken")
frm_numerical_entry = Frame(root, width=800, height=300, bd=5, bg="pink", relief="sunken")
...
def create_radiobuttons(questions_dict):
    ...
    for question in questions_dict:  # sets up labels and builds key_list from initial dictionary
        label_question = Label(frm_radiobuttons, text=question, font=("Arial", 20), fg="blue", bg="silver", height=2,
                               pady=10)
...
def create_entry_boxes():
    ...
    for y in range(len(my_tools.get_current_criteria_list())):
        entry = Entry(frm_numerical_entry, bd=4, fg="red", bg="aqua")
...
def show_result_in_label():
    ...
    lbl_result = Label(root, text="", font="Arial, 24", bg='gray', fg='yellow',
                       relief='sunken')
    lbl_result["text"] = my_tools.item + " is: \n\n " + str(my_tools.result_for_current_function) + "\n"
    lbl_result.grid(row=10, column=1, columnspan=3)
...
def combo_call_back_func(event):
    ...
    if my_tools.item in my_tools.set_of_checkbox_questions:
        frm_checkboxes.grid(row=4, column=1, columnspan=3, padx=20, pady=30)
        frm_radiobuttons.grid_forget()
    elif my_tools.item in my_tools.set_of_radiobutton_questions:
        frm_checkboxes.grid_forget()
        frm_radiobuttons.grid(row=5, column=1, columnspan=3, padx=20, pady=30)
    elif my_tools.item in my_tools.set_of_numerical_input_questions:
        frm_numerical_entry.grid(row=6, column=1, columnspan=3, padx=20, pady=30)
    ...
    if my_tools.item == "CHADS score":
        ...
        lbl_intro.grid(row=2, column=1, columnspan=3, pady=30)
...
def open_results_window():
    ...
    win_results.geometry('800x800')
    win_results.resizable(True, True)

    lbl_brief_results = Label(win_results, text=my_tools.showtext, font=("Helvetica", 16),
...
def open_details_window():
    ...
    lbl_detailed_text = Label(win_details, text="<<detailed text goes here>>", font=("Helvetica", 16),
                              wraplength=500, bg="aqua")
...
lbl_checkbox = Label(frm_checkboxes, text="Make entries:", font="Helvetica, 20", fg="blue")
lbl_checkbox.grid(row=0, column=0)

lbl_combo = Label(root, text="Scores and Calculators:", fg="blue", font="Helvetica, 24")
lbl_combo.grid(row=0, column=0, pady=10, padx=10)

lbl_combo_clinical_scenarios = Label(root, text="Clinical scenarios:", fg="blue", font="Helvetica, 24")
lbl_combo_clinical_scenarios.grid(row=0, column=2, pady=10, padx=10)

lbl_intro = Label(root, text="", font="Arial, 24", bg='gray', fg='yellow',
                  wraplength=600, relief='sunken')

# make a combo box of the topics list:

combo_scores_and_calculators = ttk.Combobox(root, value=my_tools.combobox_options_list, state="readonly")
combo_scores_and_calculators.current(0)
combo_scores_and_calculators.grid(row=0, column=1, padx=10, pady=10)
combo_scores_and_calculators.bind("<<ComboboxSelected>>", combo_call_back_func)

combo_clinical_scenarios = ttk.Combobox(root, value=my_tools.combobox_options_list, state="readonly")
combo_clinical_scenarios.current(0)
combo_clinical_scenarios.grid(row=0, column=4, padx=10, pady=10)
...
btn_open_results_window = Button(root, text="Open Results Window", font="Helvetica, 24", fg="green",
                                 command=open_results_window)
btn_open_results_window.grid(row=18, column=1, columnspan=3, pady=50)

btn_quit = Button(root, text="Exit", font="Helvetica, 24", fg="blue", command=quit)
